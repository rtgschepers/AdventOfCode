                ...
                for c in range(1, 101 - a - b):
                    ...
                    if d >= 1:
                        self.ratios['Candy'], self.ratios['Chocolate'], self.ratios['Sprinkles'], self.ratios[
                            'Sugar'] = a, b, c, d
                        if self.calc_property('calories') == 500:
                            high_score = max(high_score, self.calc_score())
        print(high_score)

    def calc_property(self, _property):
        calories = 0
        for ingredient, ratio in self.ratios.items():
            calories += self.ingredients[ingredient][_property] * ratio
        return calories
    ...
    def calc_score(self):
        ...
        for _property in self.properties:
            if _property == 'calories':
                continue
            score *= max(self.calc_property(_property), 0)
        return score
