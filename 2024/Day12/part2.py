    ...
    def solve(self):
        ...
        for y in range(len(self.garden)):
            for x in range(len(self.garden[0])):
                ...
                plots[cur_plot_id] = {'area': 0, 'perimeter': 0, 'v_edges': [], 'h_edges': []}
                ...
                while len(plot):
                    ...
                    for dx, dy in self.directions:
                        ...
                        if not (0 <= new_x < len(self.garden[0])) or not (0 <= new_y < len(self.garden)):
                            ...
                            if dx != 0 and new_x not in plots[cur_plot_id]['h_edges']:
                                plots[cur_plot_id]['h_edges'].append(new_x)
                            elif dy != 0 and new_y not in plots[cur_plot_id]['v_edges']:
                                plots[cur_plot_id]['v_edges'].append(new_y)
                            continue
        ...
        for plot in plots.values():
            ans += plot['area'] * (len(plot['h_edges']) + len(plot['v_edges']))
        print(ans)
